...
let videoSchema = new Schema(
    Video,
    {
        ...
        views: { type: 'number' },
        // An owner propery here would add support for 'channels.'
    },
    {
        dataStructure: 'JSON',
    }
...
// TODO: Integration with User and Rating Component
export async function getVideoByIdAndDelete(id) {
    try {
        let videoRepository = client.fetchRepository(videoSchema)
        await videoRepository.createIndex()
        let video = await videoRepository.remove(id);
        await videoRepository.save()
        return video
    } catch (error) {
        console.log('GetVideoByIdAndDelete Error: ', error.message)
        return []
    }
}
