    ...
    if (req.method === 'GET') {
        ...
        try {
            ...
            if (query_results) {
                return res
                    .status(200)
                    .send({ success: true, video: query_results, id: vid })
            } else {
                query_results = {}
                return res
                    .status(404)
                    .send({ success: false, video: query_results, id: vid })
            }
        } catch (error) {
            query_results = {}
            return res
        ...
        try {
            ...
            if (!user_data.role.content_editor) {
                return res
                    ...
                    .send({
                        removed: false,
                        error: 'Only Content Editors can Remove Videos.',
                    })
            }
            ...
            return res
                .status(500)
                .send({ removed: false, id: vid, message: error.message })
        }
        ...
        try {
            ...
            return res
                .status(500)
                .send({ viewed: false, error: 'Error putting view' })
        }
    } else if (req.method == 'PATCH') {
        const { filename, thumbnail } = JSON.parse(req.body)

        try {
            const add_filenames = await Video.findByIdAndUpdate(
                vid,
                { $set: { filename: filename, thumbnail: thumbnail } },
